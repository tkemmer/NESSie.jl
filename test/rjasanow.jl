...
context("laplacepot") do
    for T in testtypes
        ...
        @fact typeof(res) --> T
        @fact res --> roughly(T(4π * 0.099189377627951192068415700486459476011275034113019436460))
        ξ = elem.v2     # h = 1, φ1 = -π/4, φ2 = 0
        res = laplacepot(SingleLayer, ξ, elem, zero(T))
        @fact typeof(res) --> T
        @fact res --> roughly(T(4π * 0.070137481542397515891964948616904961601109053078512490737))
        ...
        res = laplacepot(SingleLayer, ξ, elem, zero(T))
        @fact typeof(res) --> T
        @fact res --> roughly(T(4π * 0.029051896085553676176450751869554514410165981034506945723))
        ξ = T[0, -1, -1]
        res = laplacepot(SingleLayer, ξ, elem, zero(T))
        @fact typeof(res) --> T
        @fact res --> roughly(T(4π * (0.110553267370012295141653085597608171948437903548411687443
        ...
        @fact typeof(res) --> T
        @fact res --> roughly(T(4π * 0.034775298546314300210123433855429220291493953452986787115))
        ξ = elem.v2
        res = laplacepot(SingleLayer, ξ, elem, one(T))
        @fact typeof(res) --> T
        @fact res --> roughly(T(4π * 0.03156675645153950833618064292205638527751593336549))
        ξ = elem.v3
        res = laplacepot(SingleLayer, ξ, elem, one(T))
        @fact typeof(res) --> T
        @fact res --> roughly(T(4π * 0.03156675645153950833618064292205638527751593336549))
        ...
        res = laplacepot(SingleLayer, ξ, elem, one(T))
        @fact typeof(res) --> T
        @fact res --> roughly(T(4π * (0.05486307812003175845441110089494128884416545308420259271
                                     -0.03156675645153950833618064292205638527751593336549)))
        ξ = T[0, -1, -1]
        res = laplacepot(SingleLayer, ξ, elem, one(T))
        @fact typeof(res) --> T
        @fact res --> roughly(T(4π * (2 * 0.035315594609376722705763636889615906447125307241104235442
