@testitem "Format: XML3D/.json" begin
...
@testitem "Format: XML3D/.xml" begin
    include("../testsetup.jl")
    ...
    @testset "writexml3d_xml" begin
        for T in testtypes
            # empty system
            xroot = root(parse_string(readback(fh -> Format.writexml3d_xml(fh, Vector{T}[]))))
            ...
            # small system
            nodes    = Vector{T}[T[0, 0, 0], T[0, 0, 3], T[0, 3, 0], T[1, -3, 3]]
            xroot = root(parse_string(readback(fh -> Format.writexml3d_xml(fh, nodes))))
            @test name(xroot) == "xml3d"
            xchildren = collect(child_elements(xroot))
            @test length(xchildren) == 1
            @test name(xchildren[1]) == "data"
